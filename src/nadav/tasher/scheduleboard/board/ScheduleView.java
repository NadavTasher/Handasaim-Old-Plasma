...
public class ScheduleView extends JPanel{
	...
	public ScheduleView() {
		...
		setPreferredSize(new Dimension(screen.width, screen.height*2));
	...
	private void initSchedule() {
		...
		if(!sc.isEmpty()) {
			...
			for(int c=0;c<sc.size();c++) {
				if(sc.get(c).subjects.size()>longest)longest=sc.get(c).subjects.size();
			}
	...
	public static class ClassView extends JPanel{
		...
		public ClassView(StudentClass sc,int hours) {
			classColor=new Color(new Random().nextInt(100),new Random().nextInt(100),new Random().nextInt(100));
